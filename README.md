...
[![CI](https://github.com/sergiserer/EdgarVPN/actions/workflows/ci.yml/badge.svg)](https://github.com/sergiserer/EdgarVPN/actions/workflows/ci.yml)
...
4. Sends periodic **keepalives** per relationship when idle, automatically **reconnects** — a fresh handshake with a new ephemeral key pair — if that peer goes quiet for too long, e.g. after a restart, and proactively **rotates keys** on a fixed schedule even while a session stays healthy, bounding how long any one key stays in use.
...
* Each relationship sends its own keepalives when idle; if a peer restarts (verified by actually restarting a container mid-demo) the other side notices within `SESSION_TIMEOUT_MS` and reconnects automatically with a fresh ephemeral key pair, no manual intervention. Independent of that, the initiator side of each relationship also proactively re-handshakes on a fixed schedule (`KEY_ROTATION_INTERVAL_MS`) even when the session is perfectly healthy, so no single set of data keys stays in use indefinitely — see [`docs/CRYPTOGRAPHY.md`](docs/CRYPTOGRAPHY.md) for how this differs from reconnection and what it doesn't (yet) guarantee (a brief traffic gap during the swap, not a seamless dual-session rekey).
...
* Not yet implemented: configurable keepalive/timeout/rotation intervals — see the roadmap below.
